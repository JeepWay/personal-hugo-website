...
for note_info in note_list:
    filter_tags = ['找工作',]
    target_tags = ['樹莓派','Ubuntu']
    try:
        if note_info['tags'] == []:
            logging.info(f"無標籤，跳過保存：{note_info['title']}")
            continue
        if note_info['tags'][0] in filter_tags:
            logging.info(f"標籤為 {note_info['tags'][0]}，跳過保存：{note_info['title']}")
            continue
        
        if note_info['tags'][0] not in target_tags:
            logging.info(f"標籤為 {note_info['tags'][0]}，跳過保存：{note_info['title']}")
            continue

        # 下載完整內容
        note = api.get_note(note_info['id'])
        logging.debug(note)
        ...
        front_matter = {
            ...
            "tags": note.get("tags", []),
            "draft": content_yaml.get("draft", True),
        ...
        logging.warning(f"筆記 '{note_info['title']}' 無標籤，跳過保存")
    except Exception as e:
        logging.error(f"處理筆記 '{note_info['title']}' 時發生錯誤：{str(e)}")